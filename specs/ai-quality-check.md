...
## 最终版状态
- 当前设计已确认作为第一版 UI：保留 `QualityCheck AI` icon，使用简约后台工具风格。
...
## 产品设计规范
- 首屏为质检工作台，不做营销页。
- 顶部显示应用 logo 和产品名 `QualityCheck AI`。
...
也兼容完整 `CheckResponse`，前端会重新校验 offset 并重算 `summary`。
...
- Next.js 静态产物输出到 `out/`。
- 发布包命名为 `AIqualityCheck.zip`，包内目录为 `dist/`。
- 服务器建议目录：
